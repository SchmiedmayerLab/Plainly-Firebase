...
describe("VoiceTurnQueue", () => {
  it("waits for the transcript of the latest user item", () => {
    ...
    assert.equal(queue.awaitedItemId, undefined);
  });
  ...
  it("falls back to the paraphrase when the transcript is abandoned or never existed", () => {
    const queue = new VoiceTurnQueue();
    queue.userItemAdded("item-1");
    queue.functionCalled(call);
    ...
    assert.equal(queue.take()[0]?.source, "paraphrase");
  });
  ...
  it("releases a call immediately when no user item exists yet", () => {
    const queue = new VoiceTurnQueue();
    queue.functionCalled(call);
